...
nbAgents = 6 # doit être pair et inférieur a 32
...
class AgentTypeB(object):
    ...
    def stepController(self):
        ...
        if comportement == 0:   # go straight forward
            ...
            rotationValue = (dist_array*type_array*coefs).sum() +0.39*(random()*2-1)
            translationValue = 1
        elif comportement == 6:   # go along walls
            ref = 0.48        #0.3 (0: away from wall - 1: near the wall)
            dif = 0.2     #23      (0: near the wall - 1: away from wall)
            coefs = np.array([0.6, ref, -(ref+dif), -0.7, -0.7, -(ref+dif), ref, -0.6])
            rotationValue = ( dist_array * (type_array%2) * coefs ).sum()
            translationValue = 1
...
def onExit():
    ...
    print ("\n\n=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=")
    if ret[0] > ret[1]:
        print ("Robots type A (\"" + str(AgentTypeA.teamname) + "\") wins!", abs(ret[0]-ret[1]))
    elif ret[0] < ret[1]:
        print ("Robots type B (\"" + str(AgentTypeB.teamname) + "\") wins!", abs(ret[0]-ret[1]))
    else: 
        print ("Nobody wins!")
    print ("=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n")
    print ("\n[Simulation::stop]")
