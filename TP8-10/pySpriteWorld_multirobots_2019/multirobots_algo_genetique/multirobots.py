...
class Agent(object):
    ...
    def stepController(self):
        ...
        #ar_dist = np.array([self.getDistanceAtSensor(0), self.getDistanceAtSensor(1), self.getDistanceAtSensor(2), self.getDistanceAtSensor(3), \
        #                    self.getDistanceAtSensor(4), self.getDistanceAtSensor(5), self.getDistanceAtSensor(6), self.getDistanceAtSensor(7)])
        #ar_type = np.array([self.getObjectTypeAtSensor(0), self.getObjectTypeAtSensor(1), self.getObjectTypeAtSensor(2), self.getObjectTypeAtSensor(3), \
        #                    self.getObjectTypeAtSensor(4), self.getObjectTypeAtSensor(5), self.getObjectTypeAtSensor(6), self.getObjectTypeAtSensor(7)])
        ...
        if strat == 0:
            ...
            turne = ar_dist * ar_type
            turne = np.dot(turne, [-1, -1, 1, 1])
        
            self.setRotationValue(turne)
            self.setTranslationValue(1) # normalisé -1,+1
            ...
            ar_type = np.array([self.getObjectTypeAtSensor(2), self.getObjectTypeAtSensor(3), self.getObjectTypeAtSensor(4), self.getObjectTypeAtSensor(5)])
            ar_type_new = ar_type - 1
            ar_type_new = ar_type_new * ar_type
            ar_type_new = ar_type_new / 2
            
            turne = ar_dist * ar_type_new
            rot_val = np.dot(turne, [-1, -1, 1, 1])
